...
func containerMovement(input []string) {
	for _, i := range input {
		...
		fmt.Println(pallets[Dest-1], "Source:", pallets[Source-1])
		for j := 0; j < numOfPallets; j++ {
			pallets[Dest-1] = append(pallets[Dest-1], pallets[Source-1][len(pallets[Source-1][:])-(numOfPallets-j)])
			fmt.Println(pallets[Dest-1], "Source:", pallets[Source-1])
		}
		pallets[Source-1] = pallets[Source-1][:len(pallets[Source-1][:])-numOfPallets]
		fmt.Println(pallets[Source-1])
	}
}
